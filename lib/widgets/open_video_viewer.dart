...
class _GvVideoViewerPageState extends State<_GvVideoViewerPage> {
  ...
  Future<void> _initRemoteDownloadFilePlayback() async {
    ...
    try {
      ...
      final path =
          '${dir.path}/open_video_view_${DateTime.now().millisecondsSinceEpoch}${_tempVideoSuffix(widget.videoUrl)}';
      final f = File(path);
      if (await f.exists()) {
        try {
          await f.delete();
  ...
  Future<void> _saveVideo() async {
    ...
    try {
      ...
      final folder = Directory('${documents.path}/open_chat_downloads');
