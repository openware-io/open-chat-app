...
/// 注入到 H5 的引导脚本：注册 `window.GV_SDK` 与 `GVBridge` 的 Promise 桥。
///
/// 保持与历史行为一致，仅抽离为独立常量便于主组件阅读。
const String gvMiniProgramSdkBootstrap = '''
(function(){
  if (!window.__gv_sdk_boot) {
    window.__gv_sdk_boot = true;
    var pending = {};
    ...
    function invoke(method) {
      return new Promise(function(resolve, reject) {
        var id = 'gv_' + Date.now() + '_' + Math.random().toString(36).slice(2, 11);
