...
/// `flutter run --dart-define=GV_API_BASE=http://10.0.2.2:3002
/// --dart-define=GV_WS_URI=ws://10.0.2.2:3002/ws/im/v1
/// --dart-define=GV_MEDIA_BASE=http://10.0.2.2:9000`
/// (Android emulator → host machine).
///
/// 若媒体地址为 **同机 HTTPS 自签名**（如 Vite `https://IP:5173/uploads/...`），
/// Debug 下会对与 [apiBase] **相同 host** 自动放行证书校验。
/// Profile/Release 连内网 HTTPS 时请加上：
/// `--dart-define=GV_TRUST_SELF_SIGNED=true`（仅建议内测使用）。
class AppConfig {
  AppConfig._();
  ...
  /// 极光：`flutter run --dart-define=GV_JPUSH_APPKEY=...`。
  ...
  static const String jpushDartAppKey =
      String.fromEnvironment('GV_JPUSH_APPKEY', defaultValue: '');

  /// Android 默认使用极光（包含已配置的 FCM 厂商通道）接收后台消息。
  /// 本地调试如需使用 WebSocket 前台保活回退，可显式传入
  /// `--dart-define=GV_ANDROID_JPUSH_ENABLED=false`。
  static const bool androidJPushEnabled = bool.fromEnvironment(
    'GV_ANDROID_JPUSH_ENABLED',
    defaultValue: true,
  );

  /// 默认 prod 为 true，dev/test 为 false；也可用 `GV_JPUSH_PRODUCTION` 显式覆盖。
