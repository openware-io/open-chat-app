const gvChannelCodePrefix = 'GV_CHANNEL:';

/// 频道二维码/分享 payload：`GV_CHANNEL:{code}`。
/// 与加好友码（GV_UID:）同风格，扫码时先按用户码、再按频道码依次解析。
...
/// 1. 整串以 `GV_CHANNEL:` 开头（扫码/分享 payload）；
/// 2. 文本中任意位置出现 `GV_CHANNEL:{code}`（多行分享文本）；
