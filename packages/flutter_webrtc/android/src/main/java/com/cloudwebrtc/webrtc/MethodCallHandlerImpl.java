...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  void dispose() {
    // Release every SurfaceProducer before FlutterJNI is detached. Without
    // this, a buffered ImageReader frame can call scheduleFrame on an already
    // detached engine and crash the Android process.
    for (int i = renders.size() - 1; i >= 0; i--) {
      ...
      try {
        ...
        Log.w(TAG, "dispose(): error disposing video renderer", e);
      }
    }
    renders.clear();
